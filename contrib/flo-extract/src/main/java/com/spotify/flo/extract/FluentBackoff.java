 ...
 *
 * NOTICE: Copied from from https://github.com/GoogleCloudPlatform/DataflowJavaSDK
 */
public final class FluentBackoff {
  ...
  private static final Duration DEFAULT_MIN_BACKOFF = Duration.ofSeconds(1);
  private static final Duration DEFAULT_MAX_BACKOFF = Duration.ofDays(1000);
  private static final int DEFAULT_MAX_RETRIES = Integer.MAX_VALUE;
  private static final Duration DEFAULT_MAX_CUM_BACKOFF = Duration.ofDays(1000);
  ...
  public FluentBackoff withMaxCumulativeBackoff(Duration maxCumulativeBackoff) {
    checkArgument(!maxCumulativeBackoff.isZero(),
  ...
  private static class BackoffImpl implements BackOff {
    ...
    public long nextBackOffMillis() {
      ...
      double currentIntervalMillis =
          Math.min(
              backoffConfig.initialBackoff.toMillis()
      ...
      currentCumulativeBackoff = currentCumulativeBackoff.plusMillis(nextBackoffMillis);
      currentRetry += 1;
